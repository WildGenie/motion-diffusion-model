...
def get_collate_fn(name, hml_mode='train'):
    ...
    return t2m_collate if name in ["humanml", "kit"] else all_collate


def get_dataset(name, num_frames, split='train', hml_mode='train'):
    DATA = get_dataset_class(name)
    return (
        DATA(split=split, num_frames=num_frames, mode=hml_mode)
        if name in ["humanml", "kit"]
        else DATA(split=split, num_frames=num_frames)
    )
